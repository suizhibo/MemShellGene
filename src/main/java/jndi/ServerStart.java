...
public class ServerStart {
    ...
    public static int jettyPort = 8180;
    private static Thread threadJetty;
    private static Thread threadRMI;
    private static Thread threadLDAP;
    ...
    public static ServerStart GenerateUrl(String cmd,String addrk, int port) throws Exception {
        if (addrk!=null){
            addr=addrk;
        }
        jettyPort = port;
        ServerStart servers = new ServerStart(new URL("http://"+ addr +":"+ jettyPort +"/"),StringUtils.join(cmd," "));
        Class.forName("utils.Mapper");
        return servers;
    }

    public static void getServer(ServerStart servers){
        threadJetty = new Thread(servers.jettyServer);
        threadJetty.start();

        threadRMI = new Thread(servers.rmiRefServer);
        threadRMI.start();

        threadLDAP = new Thread(servers.ldapRefServer);
        threadLDAP.start();
    }
